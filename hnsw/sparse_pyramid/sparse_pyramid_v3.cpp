...
int main(int argc, char **argv)
{
    ...
    int max_local_size;
    MPI_Reduce(&local_input_size, &max_local_size, 1, MPI_INT, MPI_MAX, 0, MPI_COMM_WORLD);

    if (rank == 0)
    {
        double distribution_rate = global_distributions / (datamatrix->global_nrow * world_size);
        std::cout << "Average activations during data distribution: " << distribution_rate << std::endl;

        double avg_local_size = static_cast<double>(local_sizes_sum) / world_size;
        double imbalance = static_cast<double>(max_local_size) / avg_local_size;
        std::cout << "Average local data size: " << avg_local_size << ", Max local data size: " << max_local_size << ", Imbalance ratio: " << imbalance << std::endl;
    ...
    // std::cout << "Rank: " << rank << " Calculated final results for " << local_query_input_size << " queries." << std::endl;
