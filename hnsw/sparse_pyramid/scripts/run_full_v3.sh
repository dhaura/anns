...
for i in {4..4}; do
    ...
    while (( K <= 2 )); do
        ...
        sed "s|__m__|$m|" | \
        sed "s|__K__|$K|" | \
        sed "s|__K_SEARCH__|$K_SEARCH|" > "$SCRATCH/benchmarks/SpKNN/anns/hnsw/sparse_pyramid/scripts/temp_full${VERSION}_${TASKS}_${K}_${K_SEARCH}.sh"

        # sbatch "$SCRATCH/benchmarks/SpKNN/anns/hnsw/sparse_pyramid/scripts/temp_full${VERSION}_${TASKS}_${K}_${K_SEARCH}.sh"
        sleep 1
        K=$((K * 2))
    done
done
