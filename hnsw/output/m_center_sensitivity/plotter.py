...
def plot_model_metrics(csv_path: str, save_path: str):
    ...
    for i, metric in enumerate(metrics):
        ...
        for model in df["model"].unique():
            ...
            grouped = model_data.groupby("m")[metric].mean().reset_index()
            ax.plot(grouped["m"], grouped[metric], marker='o', label=model)

        ax.set_title(f"Number of Kmeans Centers vs {y_labels[metric]}")
        ax.set_xlabel("Number of Kmeans Centers")
...
combine_csv_files("./raw/m_center_sensitivity", "m_center_sensitivity.csv")
plot_model_metrics("m_center_sensitivity.csv", "m_center_sensitivity_plot.png")
